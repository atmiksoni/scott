...
(function($) {
	...
	function addTab(param) {
		...
		if (param.subtitle) {
			subtitle += '<span style="color:#6B8E23;font-size:12px;"> (' + param.subtitle + ')</span>';
		}
		var html = '<div id="tabs_' + param.id + '"><a href="javascript:;" id="tabs_link_' + param.id + '" class="tab" index="' + param.id + '" closable="' + param.closable + '" hidefocus="hidefocus">' + param.title + subtitle + '</a>';
		if (param.closable == true) {
	...
	function selectTab(id) {
		if (!exists(id))
			return;
		$('div', tabsContainer).removeClass('active');
		$('.tabs-panel', panelsContainer).removeClass('active');
		$('#tabs_' + id, tabsContainer).addClass('active');
		$('#tabs_' + id + '_panel', panelsContainer).addClass('active');
	...
	function selected() {
		return $('div.active:first', tabsContainer).attr('id').substring(5);
	}
