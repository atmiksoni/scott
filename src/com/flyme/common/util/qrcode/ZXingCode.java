...
import com.flyme.thirdsdk.common.Base64;
...
public class ZXingCode {
	...
	public static void main(String[] args) throws WriterException {
		try {
			...
			getLogoQRCode("sdfs", null, "跳转到百度的二维码");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	/** * 生成带logo的二维码图片 */
	public static String getLogoQRCode(String content, String logoPath, String productName) {
		try {
			...
			return zp.addLogo_QRCode(bim, logoFile, new LogoConfig(), productName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	/** * 生成带logo的二维码图片 */
	public static String getQrcode(String content, byte[] file, String productName) {
		try {
			ZXingCode zp = new ZXingCode();
			BufferedImage bim = zp.getQR_CODEBufferedImage(content, BarcodeFormat.QR_CODE, 400, 400, zp.getDecodeHintType());
	...
	/** * 给二维码图片添加Logo */
	public String addLogo_QRCode(BufferedImage bim, byte[] logoPic, LogoConfig logoConfig, String productName) {
		try {
			/** * 读取二维码图片，并构建绘图对象 */
			...
			if (logoPic != null ) {
				...
				int widthLogo = logo.getWidth(null) > image.getWidth() * 3 / 10 ? (image.getWidth() * 3 / 10) : logo.getWidth(null), heightLogo = logo.getHeight(null) > image.getHeight() * 3 / 10 ? (image.getHeight() * 3 / 10) : logo.getWidth(null);
	...
	public Map<EncodeHintType, Object> getDecodeHintType() {
		...
		hints.put(EncodeHintType.MIN_SIZE, 400);
