...
public final class RmbUtil {
	...
	public RmbUtil(BigDecimal amount) {
		this.amount = amount;
	}
	...
	public static String toBigAmt(String amount) {
		BigDecimal bigDecimal = ObjectUtils.getBigDecimal(amount, "0.0");
		return toBigAmt(bigDecimal);
	}

	public static String toBigAmt(BigDecimal amount) {
		String amt = nf.format(amount.doubleValue());
		String dotPart = ""; // 取小数位
		String intPart = ""; // 取整数位
		int dotPos;
		...
		/*
		 * if(dotBiglength() == 0) return intBig +"元整"; else return intBig + "元"
		 * + dotBig;
		 */
	}
