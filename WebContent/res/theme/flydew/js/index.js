(function($) {
	...
	$.learuntab = {
		...
		addTab : function() {
			...
			if (dataId != "") {
				top.$.cookie('currentmoduleId', dataId, {
					path : "/"
				});
			}
		...
		loadMenu : function() {
			...
			$.each(data, function(i) {
				...
				if (row.parentId == "0") {
					...
					_html += '<a href="#">'
					_html += '<i class="' + row.icon + '"></i><span>' + row.resName + '</span><i class="fa fa-angle-left pull-right"></i>'
					_html += '</a>'
					var childNodes = $.learunindex.jsonWhere(data, function(v) {
						return v.parentId == row.resourceId
					});
					if (childNodes.length > 0) {
						...
						$.each(childNodes, function(i) {
							...
							var subchildNodes = $.learunindex.jsonWhere(data, function(v) {
								return v.parentId == subrow.resourceId
							});
							_html += '<li>';
							if (subchildNodes.length > 0) {
								_html += '<a href="#"><i class="' + subrow.icon + '"></i>' + subrow.resName + '';
								_html += '<i class="fa fa-angle-left pull-right"></i></a>';
								_html += '<ul class="treeview-menu">';
								$.each(subchildNodes, function(i) {
									var subchildNodesrow = subchildNodes[i];
									_html += '<li><a class="menuItem" data-id="' + subchildNodesrow.resourceId + '" href="' + subchildNodesrow.resUrl + '"><i class="' + subchildNodesrow.icon + '"></i>' + subchildNodesrow.resName + '</a></li>';
								});
								_html += '</ul>';

							} else {
								_html += '<a class="menuItem" data-id="' + subrow.resourceId + '" href="' + subrow.resUrl + '"><i class="' + subrow.icon + '"></i>' + subrow.resName + '</a>';
							}
							_html += '</li>';
						});
						_html += '</ul>';
